...
@router.post("/{user_id}/createorder", response_model=pmodels.orders)
def createOrder(user_id:int,db: Session=Depends(get_db)):
    ...
    order={"order_id": new_order.id, "total_price": new_order.total_price, "items": orders, "order_date": new_order.order_date, "number_of_items": len(cartItems)}
    return order
